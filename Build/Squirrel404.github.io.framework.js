...
function _Initialize() {
 SendMessage("User", "Test", "hello");
 window.addEventListener("message", e => {
  if (e.origin === "https://galaxy.click") {
   console.log(e.data);
   window.alert(e.data);
   window.alert(e.data.type);
   window.alert(e.data.action);
   if (e.data.type == "save_content") {
    window.alert("About to load: ".concat(e.data.content));
    saveData = e.data.content;
   }
  } else {}
 });
}
...
function _LoadSlot(slot = 0) {
 window.alert("Loading slot " + slot);
...
function _SaveSlot(label, saveData, slot = 0) {
 window.top.postMessage({
  action: "save",
  slot: slot,
  label: label,
